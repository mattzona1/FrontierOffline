...
func _reset_save_once() -> void:
    if FileAccess.file_exists(RESET_MARKER): return
    if FileAccess.file_exists("user://save.json"):
        DirAccess.remove_absolute(ProjectSettings.globalize_path("user://save.json"))
    var marker := FileAccess.open(RESET_MARKER, FileAccess.WRITE)
    if marker: marker.store_string("Fresh test save created for duplicate-refund milestone.")
...
func _draw_boot_screen() -> void:
    ...
    root.offset_left = 24; root.offset_right = -24; root.offset_top = 80; root.offset_bottom = -80
    ...
    var hint := Label.new()
    hint.text = "Duplicate summons refund their 5 Gems • original art available in the gallery"
...
func _add_asset_gallery(game: Node) -> Node:
    ...
    if gallery_script == null: return null
    # IMPORTANT: do not make this helper full-screen. A full-screen Control above the game
    # intercepts Android touch/scroll events even when its visible button is tiny.
    var gallery := Control.new()
    gallery.set_script(gallery_script)
    gallery.set("game", game)
    gallery.position = Vector2.ZERO
    gallery.size = Vector2(300, 270)
    gallery.mouse_filter = Control.MOUSE_FILTER_PASS
    gallery.z_index = 50
    add_child(gallery)
    return gallery
...
func _migrate_legacy_save() -> void:
    ...
    if typeof(parsed) != TYPE_DICTIONARY:
        _write_clean_save(20,1000,1,0); return
    var data: Dictionary = parsed
    if int(data.get("save_version",0)) >= SAVE_VERSION and _looks_like_v3(data): return
    _write_clean_save(maxi(0,int(data.get("gems",20))),maxi(0,int(data.get("gold",1000))),maxi(1,int(data.get("rank",1))),clampi(int(data.get("selected",0)),0,5))

func _looks_like_v3(data: Dictionary) -> bool:
    return typeof(data.get("inventory",null)) == TYPE_ARRAY and typeof(data.get("squad",null)) == TYPE_ARRAY and data["inventory"].size() >= 6 and data["squad"].size() == 6

func _write_clean_save(keep_gems:int,keep_gold:int,keep_rank:int,keep_selected:int) -> void:
    var starter_inventory := []
    for i in range(6): starter_inventory.append({"def_id":i,"level":1,"xp":0,"evo":0,"bb":0,"locked":false})
    var clean := {"save_version":SAVE_VERSION,"gems":keep_gems,"gold":keep_gold,"rank":keep_rank,"rank_xp":0,"selected":keep_selected,"unlocked_quest":0,"cleared_quests":[],"materials":{"Ember":0,"Tide":0,"Verdant":0,"Volt":0,"Lumen":0,"Dusk":0},"squad":[0,1,2,3,4,5],"inventory":starter_inventory}
    var out := FileAccess.open("user://save.json",FileAccess.WRITE)
    if out: out.store_string(JSON.stringify(clean))
