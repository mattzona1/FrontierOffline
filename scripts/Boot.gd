...
func _add_visual_backdrop() -> void:
    ...
    var backdrop := Control.new(); backdrop.set_script(backdrop_script); backdrop.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT); backdrop.mouse_filter=Control.MOUSE_FILTER_IGNORE; add_child(backdrop); move_child(backdrop,0)

func _draw_boot_screen() -> void:
    var root:=VBoxContainer.new();root.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT);root.offset_left=24;root.offset_right=-24;root.offset_top=80;root.offset_bottom=-80;root.alignment=BoxContainer.ALIGNMENT_CENTER;root.add_theme_constant_override("separation",18);add_child(root)
    var title:=Label.new();title.text="BRAVE FRONTIER";title.horizontal_alignment=HORIZONTAL_ALIGNMENT_CENTER;title.add_theme_font_size_override("font_size",44);title.add_theme_color_override("font_color",GOLD);root.add_child(title)
...
func _boot_game() -> void:
    ...
    status_label.text="Preparing unit data..."
    ...
    var game:=Control.new();game.set_script(script);game.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT);add_child(game)
    await get_tree().process_frame
...
func _add_asset_gallery(game:Node)->Node:
    ...
    var gallery:=Control.new();gallery.set_script(gallery_script);gallery.set("game",game);gallery.position=Vector2.ZERO;gallery.size=Vector2(300,270);gallery.mouse_filter=Control.MOUSE_FILTER_PASS;gallery.z_index=50;add_child(gallery);return gallery
